...
int CGameManager::GetImageColorDepth()
{
	jint Result;
	...
	if (env)
	{
		...
		jmethodID Method = env->GetStaticMethodID(Class, "GetByteCount", "()I");
		Result = static_cast<jint>(env->CallStaticIntMethod(Class, Method));
	}
	else {
		m_JavaVM->AttachCurrentThread(&env, NULL);
		jmethodID Method = env->GetStaticMethodID(g_ImageLoaderClass, "GetByteCount", "()I");
...
int CGameManager::GetImageSize(bool width)
{
    jint Result;
    ...
    JNIEnv* env;
    m_JavaVM->GetEnv((void **)&env, JNI_VERSION_1_6);
    std::string FuncName = width ? "GetWidth" : "GetHeight";
...
std::vector<uint8_t>  CGameManager::GetImageData()
{
    ...
    Result.reserve(len);
    Result.resize(len, 0);
    env->GetByteArrayRegion(Data, 0, len, reinterpret_cast<jbyte*>(&Result[0]));
...
bool CGameManager::LoadImageData(const char* path)
{
#ifdef PLATFORM_ANDROID
